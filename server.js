...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (error.status || error.message === 'Fixture not found' || error.message.startsWith('Fixture name') || error.message.startsWith('Method must') || error.message.startsWith('Target URL') || error.message.startsWith('Replay target') || error.message.startsWith('Request body') || error.message.startsWith('Invalid JSON') || error.message.startsWith('Import payload')) {
